...
'{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')""".format(
        ...
        metric_insert.FN,
        metric_insert.TP,
        metric_insert.precision,
        metric_insert.recall,
        metric_insert.fbeta,
        metric_insert.pr_auc,
        metric_insert.roc_auc,
        metric_insert.cks,
    )
    ...
    try:
        ...
        connSQL = pymssql.connect(
            server=lines[0], user=lines[1], password=lines[2], database=lines[3]
        )
...
def regular_metrics(score, abnormal_segment, np_decision, pos_label):
    ...
    fbeta = fbeta_score(
        y_true=abnormal_segment, y_pred=np_decision, pos_label=pos_label, beta=1
    )
...
def bestF1_metrics(score, abnormal_segment, pos_label, adjusted_points):
    ...
    metrics = np.array(
        [["Thresholds", "Precision", "Recall", "F1", "CKS", "TN", "FP", "FN", "TP"]]
    )
    ...
    top_f1 = (
        pd.DataFrame(data=metrics[1:, 0:], columns=metrics[0, 0:])
        .apply(pd.to_numeric)
        .nlargest(1, ["F1"])
    )
    metrics = pd.DataFrame(data=metrics[1:, 0:], columns=metrics[0, 0:]).apply(
        pd.to_numeric
    )
...
def at_k_metrics(score, abnormal_segment, pos_label, at_k, adjusted_points):
    ...
    np_decision = pd.DataFrame(y_predicted, columns=["Score"]).iloc[
        0 : abnormal_segment.shape[0], 0
    ]
...
def CalculateMetrics(
    ...
    for adjusted_points in (False, True):
        ...
        for std_dev in std_dev_values:
            ...
            zscore_metrics = regular_metrics(
                score, abnormal_segment, np_decision, pos_label
            )
            insert_SQL(
                zscore_metrics,
                model_name,
        ...
        for quantile in quantile_values:
            ...
            quantile_metrics = regular_metrics(
                score, abnormal_segment, np_decision, pos_label
            )
            insert_SQL(
                quantile_metrics,
                ...
                dataset,
                file_name,
                "Percentile: " + str(quantile),
                adjusted_points,
            )

        # @K
        for at_k in k_values:
            k_metrics = at_k_metrics(
                score, abnormal_segment, pos_label, at_k, adjusted_points
            )
            insert_SQL(
                k_metrics,
                model_name,
        ...
        # Best F1
        bestf1_metrics = bestF1_metrics(
            score, abnormal_segment, pos_label, adjusted_points
        )
        insert_SQL(
            bestf1_metrics,
            model_name,
