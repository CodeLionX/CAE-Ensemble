...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        for i, convolution in enumerate(self.convolutions):
            convolution_hidden = F.glu(
                convolution(self.dropout(convolution_input)), dim=1
            )
...
class Decoder(nn.Module):
    ...
    def __init__(self, x_dim, h_dim, sequence_length, dropout):
        ...
        self.convolutions = nn.ModuleList(
            [
                nn.Conv1d(in_channels=h_dim * 2, out_channels=h_dim * 4, kernel_size=3)
                for _ in range(5)
            ]
        )
        self.to_hidden = nn.Linear(h_dim_half, h_dim * 2)
    ...
    def calculate_attention(
        ...
        embedded,
        convolution_hidden,
        encoder_convolution_hidden,
        encoder_residual_connection,
    ):
        residual_connection = (
            self.attention_from_hidden(convolution_hidden.permute(0, 2, 1)) + embedded
        )
        attention = F.softmax(
            torch.matmul(
                residual_connection, encoder_convolution_hidden.permute(0, 2, 1)
            ),
            dim=2,
        )
        attention_encoded = self.attention_to_hidden(
            torch.matmul(attention, encoder_residual_connection)
        )
    ...
    def forward(self, target, hidden_encoder, residual_encoder):
        ...
        for i, convolution in enumerate(self.convolutions):
            ...
            padding = (
                torch.zeros(convolution_input.size()[0], self.h_dim * 2, 2)
                .fill_(1)
                .to(device)
            )
            ...
            attention, convolution_hidden = self.calculate_attention(
                embedded, convolution_hidden, hidden_encoder, residual_encoder
            )
            convolution_hidden = convolution_hidden + convolution_input
            convolution_input = convolution_hidden
...
class DiversityLoss(nn.Module):
    ...
    def forward(self, input, target, batch_number, batch_size):
        ...
        for i in range(len(self.ensemble_member)):
            batch_segment = self.ensemble_member[i].train_means[
                batch_number * batch_size : batch_number * batch_size + size_input
            ]
            norm_results += (
                math.sqrt(2) / 2 * torch.norm(input - batch_segment, 2) / size_input
            )

        loss_diversity = self.loss_function(
            input, target
        ) + self.diversity_factor * norm_results / len(self.ensemble_member)
...
class CAE(nn.Module):
    def __init__(self, file_name, config):
        ...
        if self.save_model:
            ...
            self.save_model_path = "./save_models/{}/CAE_ENSEMBLE_{}_pid={}.pt".format(
                self.dataset, Path(self.file_name).stem, self.pid
            )
        else:
        ...
        if self.load_model:
            self.load_model_path = "./save_models/{}/CAE_ENSEMBLE_{}_pid={}.pt".format(
                self.dataset, Path(self.file_name).stem, self.pid
            )
        else:
        ...
        self.encoder = Encoder(
            x_dim=self.x_dim,
            h_dim=self.h_dim,
            sequence_length=self.rolling_size,
    ...
    def fit(
        ...
        TN, TP, FN, FP, PRECISION, RECALL, FBETA, PR_AUC, ROC_AUC, CKS = (
            [] for _ in range(10)
        )
        ...
        sched = lr_scheduler.StepLR(
            optimizer=opt, step_size=self.milestone_epochs, gamma=self.gamma
        )
            ...
            for epoch in range(self.epochs):
                ...
                for i, (batch_x, batch_y) in enumerate(train_data):
                    ...
                    if len(ensemble_member) == 0:
                        ...
                        batch_loss = loss_fn(
                            batch_x_reconstruct, batch_x, i, self.batch_size
                        )

                    batch_loss.backward()
                    if self.use_clip_norm:
                        torch.nn.utils.clip_grad_norm_(
                            list(self.parameters()), self.gradient_clip_norm
                        )
                ...
                if epoch % self.display_epoch == 0:
                    train_logger.info(
                        "epoch = {} , train loss = {}".format(epoch, epoch_losses[-1])
                    )
                    if self.dataset != -1:
                        self.eval()
                        with torch.no_grad():
                            ...
                            cae_output = CAEOutput(
                                dec_means=cat_xs,
                                train_means=None,
                                validation_means=None,
                                best_TN=None,
                                best_FP=None,
                                best_FN=None,
                                best_TP=None,
                            ...
                            if self.preprocessing:
                                    ...
                                    if self.use_last_point:
                                        ...
                                        dec_mean_unroll = unroll_window_3D(
                                            np.reshape(
                                                ...
                                                .cpu()
                                                .numpy(),
                                                (-1, self.rolling_size, original_x_dim),
                                            )
                                        )[::-1]
                                        dec_mean_unroll = min_max_scaler.fit_transform(
                                            dec_mean_unroll
                                ...
                                dec_mean_unroll = np.transpose(
                                    np.squeeze(dec_mean_unroll, axis=0)
                                )
                                dec_mean_unroll = min_max_scaler.fit_transform(
                                    dec_mean_unroll
                                )
                            ...
                            np_decision = create_label_based_on_zscore(
                                final_zscore, 2.5, True
                            )

                            pos_label = -1

                            precision = precision_score(
                                y_true=abnormal_segment,
                                y_pred=np_decision,
                                pos_label=pos_label,
                            )
                            recall = recall_score(
                            ...
                            pre, re, _ = precision_recall_curve(
                                y_true=np.nan_to_num(abnormal_segment),
                        ...
                        if config.validation == 3:
                            ...
                            if config.preprocessing:
                                if config.use_overlapping:
                                    if config.use_last_point:
                                        ...
                                        validation_unroll = unroll_window_3D(
                                            np.reshape(
                                                val_train.detach().cpu().numpy(),
                                                (
                                                    -1,
                                                    config.rolling_size,
                                                    original_x_dim,
                                        ...
                                        validation_original = unroll_window_3D(
                                            np.reshape(
                                                val_input,
                                                (
                                                    -1,
                                    ...
                                    validation_unroll = np.reshape(
                                        val_train.detach().cpu().numpy(),
                                        (-1, original_x_dim),
                                    )
                                    validation_unroll = min_max_scaler.fit_transform(
                                        validation_unroll
                                    )
                                    validation_original = np.reshape(
                                        val_input, (-1, original_x_dim)
                                    )

                            else:
                                validation_unroll = val_train.detach().cpu().numpy()
                                validation_unroll = np.transpose(
                                    np.squeeze(validation_unroll, axis=0)
                                )
                                validation_unroll = min_max_scaler.fit_transform(
                                    validation_unroll
                                )
                            ...
                            validation_original = scaler.fit_transform(
                                validation_original
                            )
                            ...
                            validation_error = (
                                np.sum(
                                    validation_original
                                    - np.reshape(
                                        validation_unroll, [-1, original_x_dim]
                                    ),
                                    axis=1,
                                )
                                ** 2
                            ...
                            error_by_timestamp = (
                                np.sum(validation_error) / validation_original.shape[0]
                            )
        ...
        with torch.no_grad():
            ...
            if self.validation:
                ...
                cae_output = CAEOutput(
                    dec_means=cat_xs,
                    train_means=cat_train,
                    validation_means=val_train,
                    best_TN=max(TN) if TN else 0.0,
                    best_FP=max(FP) if FP else 0.0,
                    best_FN=max(FN) if FN else 0.0,
                    best_TP=max(TP) if TP else 0.0,
                    best_precision=max(PRECISION) if PRECISION else 0.0,
                    best_recall=max(RECALL) if RECALL else 0.0,
                    best_fbeta=max(FBETA) if FBETA else 0.0,
                    best_pr_auc=max(PR_AUC) if PR_AUC else 0.0,
                    best_roc_auc=max(ROC_AUC) if ROC_AUC else 0.0,
                    best_cks=max(CKS) if CKS else 0.0,
                )
            else:
                cae_output = CAEOutput(
                    dec_means=cat_xs,
...
class CAE_Ensemble(nn.Module):
    ...
    def fit(
        ...
        for member in range(self.members):
            generic_features = np.empty((0,), dtype=np.int_)
            if member > 1:
                parameters_last_model = len(list(self.cae.parameters()))
                generic_features = random.sample(
                    range(1, parameters_last_model),
            ...
            train_logger.info(f"Fitting member {member} with {generic_features=}")
            self.outputs.append(
                self.cae.fit(
                    train_input,
                    val_input,
...
def spot_thread(abnormal_segment, error, config, cae_output):
    ...
    precision = precision_score(
        y_true=abnormal_segment, y_pred=np_decision, pos_label=pos_label
    )
    recall = recall_score(
        y_true=abnormal_segment, y_pred=np_decision, pos_label=pos_label
    )
    fbeta = fbeta_score(
        y_true=abnormal_segment, y_pred=np_decision, pos_label=pos_label, beta=0.5
    )
    fpr, tpr, _ = roc_curve(
        y_true=np.nan_to_num(abnormal_segment),
        y_score=np.nan_to_num(error),
    ...
    '{}', '{}')""".format(
        ...
        cae_output.best_FN,
        cae_output.best_TP,
        cae_output.best_precision,
        cae_output.best_recall,
        cae_output.best_fbeta,
        cae_output.best_pr_auc,
        cae_output.best_roc_auc,
        cae_output.best_cks,
    )
...
def RunModel(file_name, config):
    train_data = None
    if config.dataset == -1:
        abnormal_data, abnormal_label = read_autotsad_dataset(file_name)
    if config.dataset == 0:
        abnormal_data, abnormal_label = generate_synthetic_dataset(
            case=0, N=512, noise=True, verbose=False
        )
    if config.dataset == 1:
    ...
    if (
        config.dataset == 31
        or config.dataset == 32
        or config.dataset == 33
        or config.dataset == 34
        or config.dataset == 35
    ):
        abnormal_data, abnormal_label = read_S5_dataset(
            file_name, include_slope=config.use_slope, slope=config.slope
        )
    if (
        config.dataset == 41
        or config.dataset == 42
        or config.dataset == 43
        or config.dataset == 44
        or config.dataset == 45
        or config.dataset == 46
    ):
        abnormal_data, abnormal_label = read_NAB_dataset(file_name)
    if (
        config.dataset == 51
        or config.dataset == 52
        or config.dataset == 53
        or config.dataset == 54
        or config.dataset == 55
        or config.dataset == 56
        or config.dataset == 57
    ):
        train_data, abnormal_data, abnormal_label = read_2D_dataset(file_name)
    if (
        config.dataset == 61
        or config.dataset == 62
        or config.dataset == 63
        or config.dataset == 64
        or config.dataset == 65
        or config.dataset == 66
        or config.dataset == 67
    ):
        abnormal_data, abnormal_label = read_ECG_dataset(
            file_name, include_slope=config.use_slope, slope=config.slope
        )
    if config.dataset == 71 or config.dataset == 72 or config.dataset == 73:
        train_data, abnormal_data, abnormal_label = read_SMD_dataset(
            file_name, include_slope=config.use_slope, slope=config.slope
        )
    if (
        config.dataset == 81
        or config.dataset == 82
        or config.dataset == 83
        or config.dataset == 84
        or config.dataset == 85
        or config.dataset == 86
        or config.dataset == 87
        or config.dataset == 88
        or config.dataset == 89
        or config.dataset == 90
    ):
        train_data, abnormal_data, abnormal_label = read_SMAP_dataset(
            file_name, include_slope=config.use_slope, slope=config.slope
        )
    if (
        config.dataset == 91
        or config.dataset == 92
        or config.dataset == 93
        or config.dataset == 94
        or config.dataset == 95
        or config.dataset == 96
        or config.dataset == 97
    ):
        train_data, abnormal_data, abnormal_label = read_MSL_dataset(
            file_name, include_slope=config.use_slope, slope=config.slope
        )
    if config.dataset == 101:
        train_data, abnormal_data, abnormal_label = read_SWAT_dataset(
            file_name, sampling=0.1, include_slope=config.use_slope, slope=config.slope
        )
    if config.dataset == 102 or config.dataset == 103:
        train_data, abnormal_data, abnormal_label = read_WADI_dataset(
            file_name, sampling=0.1, include_slope=config.use_slope, slope=config.slope
        )
    ...
    if config.preprocessing:
        if config.use_overlapping:
            if train_data is not None:
                rolling_train_data, rolling_abnormal_data, rolling_abnormal_label = (
                    rolling_window_2D(train_data, config.rolling_size),
                ...
                rolling_abnormal_data, rolling_abnormal_label = rolling_window_2D(
                    abnormal_data, config.rolling_size
                ), rolling_window_2D(abnormal_label, config.rolling_size)
        else:
            if train_data is not None:
                rolling_train_data, rolling_abnormal_data, rolling_abnormal_label = (
                    cutting_window_2D(train_data, config.rolling_size),
        ...
        if train_data is not None:
            ...
            rolling_abnormal_data, rolling_abnormal_label = (
                abnormal_data,
                abnormal_label,
            )
    ...
    if train_data is not None and config.robustness == False:
        cae_output = model.fit(
            train_input=rolling_train_data,
            val_input=rolling_train_data,
        ...
        cae_output = model.fit(
            train_input=rolling_abnormal_data,
            val_input=rolling_abnormal_data,
            test_input=rolling_abnormal_data,
            test_label=rolling_abnormal_label,
            abnormal_data=abnormal_data,
            abnormal_label=abnormal_label,
    ...
    for ensemble_index, cae_output in enumerate(cae_outputs):
            ...
            if config.use_overlapping:
                if config.use_last_point:
                    ...
                    dec_mean_unroll = unroll_window_3D(
                        np.reshape(
                            cae_output.dec_means.detach().cpu().numpy(),
                            (-1, config.rolling_size, original_x_dim),
                        )
                    )[::-1]
                    dec_mean_unroll = min_max_scaler.fit_transform(dec_mean_unroll)
                ...
                dec_mean_unroll = np.reshape(
                    cae_output.dec_means.detach().cpu().numpy(), (-1, original_x_dim)
                )
                dec_mean_unroll = min_max_scaler.fit_transform(dec_mean_unroll)
                x_original_unroll = abnormal_data[: dec_mean_unroll.shape[0]]
                abnormal_segment = abnormal_label[: dec_mean_unroll.shape[0]]
        ...
        error = (
            np.sum(
                x_original_unroll - np.reshape(dec_mean_unroll, [-1, original_x_dim]),
                axis=1,
            )
            ** 2
        )
        ...
        if config.save_figure:
            ...
            if original_x_dim == 1:
                ...
                # plt.show()
                plt.savefig(
                    "./save_figures/{}/Ori_CAE_ENSEMBLE_{}_pid={}.png".format(
                        config.dataset, Path(file_name).stem, config.pid
                    ),
                    dpi=300,
                )
                plt.close()
                ...
                # plt.show()
                plt.savefig(
                    "./save_figures/{}/Dec_CAE_ENSEMBLE_{}_pid={}.png".format(
                        config.dataset, Path(file_name).stem, config.pid
                    ),
                    dpi=300,
                )
                ...
                # plt.show()
                plt.savefig(
                    "./save_figures/{}/VisInp_CAE_ENSEMBLE_{}_pid={}.png".format(
                        config.dataset, Path(file_name).stem, config.pid
                    ),
                    dpi=300,
                )
                ...
                # plt.show()
                plt.savefig(
                    "./save_figures/{}/VisOut_CAE_ENSEMBLE_{}_pid={}.png".format(
                        config.dataset, Path(file_name).stem, config.pid
                    ),
                    dpi=300,
                )
        ...
        try:
            ...
            # add padding and store for later
            if config.preprocessing:
                if config.use_overlapping:
                    if config.use_last_point:
                        file_logger.info(
                            f"Unrolling using last point ({config.rolling_size - 1}, 0)"
                        )
                        pad_shape = (config.rolling_size - 1, 0)
                    else:
                        file_logger.info(
                            f"Unrolling using overlapping (0, {dec_mean_unroll.shape[0]})"
                        )
                        pad_shape = (0, dec_mean_unroll.shape[0])
                else:
                    file_logger.info(
                        f"Unrolling using preprocessing (0, {dec_mean_unroll.shape[0]})"
                    )
                    pad_shape = (0, dec_mean_unroll.shape[0])
                pad_score = np.pad(
                    final_zscore,
                ...
                file_logger.info("No unrolling")
                pad_score = final_zscore
            assert (
                pad_score.shape[0] == abnormal_data.shape[0]
            ), f"{pad_score.shape=} != {abnormal_data.shape=}"
            ensemble_member_scores.append(pad_score)

            if config.dataset != -1:
                # we do not need the metrics for autotsad

                np_decision = create_label_based_on_zscore(final_zscore, 2.5, True)
                cm = confusion_matrix(
                    y_true=abnormal_segment, y_pred=np_decision, labels=[1, -1]
                )
                precision = precision_score(
                    y_true=abnormal_segment, y_pred=np_decision, pos_label=pos_label
                )
                ...
                pre, re, _ = precision_recall_curve(
                    y_true=np.nan_to_num(abnormal_segment),
                    probas_pred=np.nan_to_num(score),
                    pos_label=pos_label,
                )
                pr_auc = auc(re, pre)
                fpr, tpr, _ = roc_curve(
                    y_true=np.nan_to_num(abnormal_segment),
                    y_score=np.nan_to_num(score),
                    pos_label=pos_label,
                )
                roc_auc = auc(fpr, tpr)
                cks = cohen_kappa_score(y1=abnormal_segment, y2=np_decision)
    ...
    # save median anomaly score:
    if config.save_output:
        folder = Path("./save_outputs/scores") / str(config.dataset)
        folder.mkdir(parents=True, exist_ok=True)
        np.savetxt(
            folder / f"CAE_ENSEMBLE_{Path(file_name).stem}.csv",
            np.median(ensemble_member_scores, axis=0),
            delimiter=",",
        )
...
def RunValidation(file_name, config):
    ...
    if train_data is not None and config.robustness == False:
        rolling_train_data, validation_data = np.split(
            rolling_train_data, [int(0.7 * len(rolling_train_data))]
        )
        cae_output = model.fit(
            train_input=rolling_train_data,
        ...
        rolling_abnormal_data_cut, validation_data = np.split(
            rolling_abnormal_data, [int(0.7 * len(rolling_abnormal_data))]
        )
        cae_output = model.fit(
            train_input=rolling_abnormal_data_cut,
            val_input=validation_data,
            test_input=rolling_abnormal_data,
            test_label=rolling_abnormal_label,
            abnormal_data=abnormal_data,
    ...
    for ensemble_index, cae_output in enumerate(cae_outputs):
        ...
        if config.preprocessing:
            if config.use_overlapping:
                if config.use_last_point:
                    ...
                    validation_unroll = (
                        cae_output.validation_means.detach().cpu().numpy()[:, -1]
                    )
                    ...
                    validation_unroll = unroll_window_3D(
                        np.reshape(
                            cae_output.validation_means.detach().cpu().numpy(),
                            (-1, config.rolling_size, original_x_dim),
                        )
                    )[::-1]
                    validation_unroll = min_max_scaler.fit_transform(validation_unroll)
                    validation_original = unroll_window_3D(
                        np.reshape(
                            validation_data, (-1, config.rolling_size, original_x_dim)
                        )
                    )[::-1]
            else:
                dec_mean_unroll = np.reshape(
        ...
        validation_error = (
            np.sum(
                validation_original
                - np.reshape(validation_unroll, [-1, original_x_dim]),
                axis=1,
            )
            ** 2
        )
        ...
        ensemble_member_scores.append(pad_score)
        ensemble_member_val_scores.append(validation_error)
    ...
    if config.save_output:
        folder = Path("./save_outputs/scores") / str(config.dataset)
        folder.mkdir(parents=True, exist_ok=True)
        beta = config.beta_transfer
        lambda_ = config.diversity_factor
        w = config.rolling_size
        np.savetxt(
            folder / f"CAE_ENSEMBLE_{Path(file_name).stem}_{w=}_{beta=}_{lambda_=}.csv",
            np.median(ensemble_member_scores, axis=0),
            delimiter=",",
        )

    # validation error for hyperparameter tuning
    average_val_error = (
        np.sum(np.median(ensemble_member_val_scores, axis=0))
        / validation_original.shape[0]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hyperparameter-samples", type=int, default=10)
    ...
    if args.load_config:
        ...
        try:
            config.import_config(
                "./save_configs/{}/Config_CAE_ENSEMBLE_pid={}.json".format(
                    config.dataset, config.pid
                )
            )
        except:
        ...
        config = CAEConfig(
            ...
            use_overlapping=args.use_overlapping,
            rolling_size=args.rolling_size,
            ensemble=args.ensemble,
            ensemble_members=args.ensemble_members,
            beta_transfer=args.beta_transfer,
            ...
            adjusted_points=args.adjusted_points,
            continue_training=args.continue_training,
            dropout=args.dropout,
            use_spot=args.use_spot,
            use_last_point=args.use_last_point,
            save_config=args.save_config,
            load_config=args.load_config,
            server_run=args.server_run,
            robustness=args.robustness,
            pid=args.pid,
            use_clip_norm=args.use_clip_norm,
    ...
    file_logger.info(f"Using torch device {device}")
    ...
    if args.dataset == -1:
        # only use the supplied dataset for autotsad
        all_files = [(path, [], [args.dataset_path.name])]
    else:
    ...
    for root, dirs, files in all_files:
        if len(dirs) == 0:
            ...
            s_best_TN, s_best_FP, s_best_FN, s_best_TP, s_best_precision = (
                [] for _ in range(5)
            )
            (
                s_best_recall,
            ...
            for file in files:
                ...
                if config.validation == 1:  # Beta, lambda, windows size
                    if args.dataset != -1:
                        for beta_validation in [float(j) / 10 for j in range(1, 10, 1)]:
                            ...
                            for lambda_validation in (1, 2, 4, 8, 16, 32, 64):
                                ...
                                for windows_validation in (4, 8, 16, 32, 64, 128, 256):
                                    ...
                                    metric_result = RunValidation(file_name, config)
                    else:
                        import shutil
                        from itertools import product

                        # perform hyperparameter optimization on a sampled number of hyperparameters
                        val_errors = {}
                        validation_params = product(
                            [float(j) / 10 for j in range(1, 10, 1)],
                            (1, 2, 4, 8, 16, 32, 64),
                            (4, 8, 16, 32, 64, 128, 256),
                        )
                        sampled_params = random.sample(
                            list(validation_params), args.hyperparameter_samples
                        )
                        meta_logger.info(f"Sampled hyperparameters: {sampled_params}")
                        for (
                            beta_validation,
                            lambda_validation,
                            windows_validation,
                            ...
                            train_logger.info(
                                f"Fitting model with (beta,lambda,w)={key}"
                            )
                            val_errors[key] = RunValidation(
                                file_name=file_name, config=config
                            )

                        # Chose the best hyperparameters based on median reconstruction error
                        sorted_errors = sorted(
                            list(val_errors.items()), key=lambda x: x[1]
                        )
                        (median_beta, median_lambda, median_window), median_error = (
                            sorted_errors[len(sorted_errors) // 2]
                        )
                        meta_logger.info(
                            f"Best hyperparameters: beta={median_beta}, lambda={median_lambda}, window={median_window}"
                        )
                        errors = ",".join([f"{v:.4f}" for k, v in sorted_errors])
                        meta_logger.info(
                            f"Chosen median error: {median_error} from {errors}"
                        )
                        ...
                        source_path = (
                            ...
                            / "scores"
                            / str(args.dataset)
                            / f"CAE_ENSEMBLE_{Path(file_name).stem}_w={median_window}_beta={median_beta}_lambda_={median_lambda}.csv"
                        )
                        target_path = (
                            source_path.parent
                            / f"CAE_ENSEMBLE_{Path(file_name).stem}.csv"
                        )
                    ...
                    if args.dataset == -1:
                        # only process the provided dataset for autotsad, we also do not
                        # need the metrics because we are only interested in the anomaly
                        # scoring
                        file_logger.info("Finish")
                        meta_logger.info("Finish")
                        sys.exit(0)
                ...
                file_logger.info(
                    "best_precision = {}".format(metrics_result.best_precision)
                )
                ...
                file_logger.info(
                    "best_roc_auc = {}".format(metrics_result.best_roc_auc)
                )
